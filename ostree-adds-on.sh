...
ARCH=$(uname -m)
TEST_UUID=$(uuidgen)
IMAGE_KEY="ostree-adds-on-${TEST_UUID}"
...
TEMPDIR=$(mktemp -d)
BLUEPRINT_FILE="adds-on/${USER_CASE}.toml"
COMPOSE_START=${TEMPDIR}/compose-start-${IMAGE_KEY}.json
COMPOSE_INFO=${TEMPDIR}/compose-info-${IMAGE_KEY}.json
KS_FILE="adds-on/${USER_CASE}-ks.cfg"
...
case "${ID}-${VERSION_ID}" in
    "rhel-9.2")
        ...
        BOOT_LOCATION="http://${DOWNLOAD_NODE}/rhel-9/nightly/RHEL-9/latest-RHEL-9.2.0/compose/BaseOS/x86_64/os/"
        ;;
    *)
...
get_compose_log () {
    COMPOSE_ID=$1
    LOG_FILE=osbuild-${ID}-${VERSION_ID}-${USER_CASE}-${COMPOSE_ID}.log
...
get_compose_metadata () {
    COMPOSE_ID=$1
    METADATA_FILE=osbuild-${ID}-${VERSION_ID}-${USER_CASE}-${COMPOSE_ID}.json
...
greenprint "📑 which kickstart file is using"
cat "$KS_FILE"
########################################################
##
...
sudo virt-install  --initrd-inject="${KS_FILE}" \
                   --extra-args="inst.ks=file:/${USER_CASE}-ks.cfg console=ttyS0,115200" \
                   --name="${IMAGE_KEY}-bios"\
...
# Test IoT/Edge OS
podman run -v "$(pwd)":/work:z -v "${TEMPDIR}":/tmp:z --rm quay.io/rhel-edge/ansible-runner:latest ansible-playbook -v -i /tmp/inventory "adds-on/${USER_CASE}.yaml" || RESULTS=0
check_result
...
########################################
##
## Upgrade test with rpm-ostree rebase
##
########################################

# Build installer image.
build_image container "${CONTAINER_IMAGE_TYPE}" "$OSTREE_REBASE_REF" "$PROD_REPO_URL" "$PARENT_REF"
...
sudo ostree --repo="$PROD_REPO" pull --mirror edge-stage "$OSTREE_REBASE_REF"
...
########################################
##
## Upgrade test with rpm-ostree upgrade
##
########################################

# Build installer image.
build_image container "${CONTAINER_IMAGE_TYPE}" "$OSTREE_REF" "$PROD_REPO_URL"
