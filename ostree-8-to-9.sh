...
sudo dnf install -y --nogpgcheck osbuild osbuild-composer composer-cli ansible-core python-jmespath podman firewalld httpd
...
get_compose_log () {
    COMPOSE_ID=$1
    LOG_FILE=osbuild-${ID}-${VERSION_ID}-8-9-${COMPOSE_ID}.log
...
get_compose_metadata () {
    COMPOSE_ID=$1
    METADATA_FILE=osbuild-${ID}-${VERSION_ID}-8-9-${COMPOSE_ID}.json
