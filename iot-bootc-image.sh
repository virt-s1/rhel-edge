...
if ! wait_for_ssh "${GUEST_IP}"; then
    ...
    sudo virsh dumpxml "iot-bootc-image-${TEST_UUID}" | grep -A10 '<interface' || true
    ...
    sudo journalctl -u libvirtd --since '30 min ago' --no-pager -p warning | tail -50 || true
    exit 1
fi
