...
############################################################
##
## Build iot-qcow2-image
##
############################################################

# Write a blueprint for qcow2 image.
tee "$BLUEPRINT_FILE" > /dev/null << EOF
name = "iot-qcow2"
...
# Prepare the blueprint for the compose.
greenprint "📋 Preparing qcow2 image blueprint"
sudo composer-cli blueprints push "$BLUEPRINT_FILE"
...
##################################################################
##
## Install and test edge vm with iot-qcow2-image (UEFI)
##
##################################################################
...
# Test IoT/Edge OS
podman run --annotation run.oci.keep_original_groups=1 -v "$(pwd)":/work:z -v "${TEMPDIR}":/tmp:z --rm quay.io/rhel-edge/ansible-runner:latest ansible-playbook -v -i /tmp/inventory \
    -e os_name="${OSTREE_OSNAME}" \
